...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-[1100px] border border-[var(--navbar-border)] rounded-[12px] px-4 sm:px-6 py-3 flex items-center justify-between gap-3 pointer-events-auto bg-[var(--navbar-bg)] backdrop-blur-[20px] shadow-[var(--card-shadow)] ring-1 ring-[var(--purple-500)]/10">
        {/* Logo */}
        <Link href="/" className="group flex items-center gap-2.5 transition-all hover:scale-[1.02]">
          <div className="relative w-8 h-8 md:w-9 md:h-9 shrink-0 drop-shadow-[0_0_8px_rgba(99,103,255,0.3)]">
            <Image 
          ...
          <span className="font-syne font-bold text-[18px] md:text-[20px] tracking-tight text-[var(--text-primary)] group-hover:text-[var(--purple-500)] transition-colors">
        ...
        <div className="flex items-center gap-3">
              ...
              return (
                <Link
                  ...
                  className={`relative text-[13px] sm:text-sm transition-all py-1 ${
                    isActive
                      ? "text-[var(--text-primary)] font-semibold drop-shadow-[0_0_8px_rgba(99,103,255,0.4)]"
                      : "text-[var(--text-primary)] opacity-60 hover:opacity-100"
                  }`}
                >
                  {link.name}
                  {isActive && (
                    <span className="absolute -bottom-1 left-0 w-full h-0.5 bg-[var(--purple-500)] rounded-full shadow-[0_0_10px_rgba(99,103,255,0.5)]" />
                  )}
                </Link>
              );
            })}
          ...
          <Link
            ...
            className={`transition-all inline-block hover:scale-110 active:scale-95 ${
              pathname === "/search"
                ? "text-[var(--purple-500)] drop-shadow-[0_0_10px_rgba(99,103,255,0.5)]"
                : "text-[var(--text-primary)] opacity-60 hover:opacity-100"
            }`}
            aria-label="Search"
          >
                ...
                return (
                  <Link
                    ...
                    className={`px-4 py-3 rounded-[10px] text-[14px] font-medium transition-all ${
                      isActive
                        ? "text-[var(--text-primary)] bg-[var(--purple-500)]/10 border border-[var(--purple-500)]/20"
                        : "text-[var(--text-primary)] opacity-60 hover:opacity-100 hover:bg-[var(--bg-hover)]"
                    }`}
                  >
                    {link.name}
                  </Link>
