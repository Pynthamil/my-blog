...
export default function SyntaxHighlighter() {
  useEffect(() => {
    ...
    // before attempting to highlight and add buttons
    const timeout = setTimeout(() => {
      ...
      codeBlocks.forEach((block) => {
        ...
        if (pre && pre.tagName === "PRE" && !pre.querySelector(".copy-button")) {
          ...
          button.className = "copy-button absolute top-2 right-2 w-8 h-8 flex items-center justify-center rounded-lg bg-black/40 backdrop-blur-md border border-white/10 hover:bg-black/60 hover:border-white/20 transition-all group z-10 cursor-pointer";
